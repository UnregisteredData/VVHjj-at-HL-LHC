...
SigGRAPH_BOOSTED = TH1D("Signal1", "tagjet2eta: Boosted Selection", 50, 0, 700)
BackGRAPH_BOOSTED = TH1D("Signal2", "tagjet2eta: Boosted Selection", 50, 0, 700)
SigGRAPH_HYBRID = TH1D("Signal3", "tagjet2eta: Hybrid Selection", 50, 0, 700)
BackGRAPH_HYBRID = TH1D("Signal4", "tagjet2eta: Hybrid Selection", 50, 0, 700)
...
for jentry in range( entries ):
    ...
    if any(pt > 200 for pt in mychain1.fatjetspt) and any(eta < 2 for eta in mychain1.fatjetseta) and any(Rpt > 20 for Rpt in mychain1.smallRjetspt) and mhiggs > 70 and mhiggs < 140:
        #BOOSTED
        if (nfatjets > 1) and any(mass > 30 for mass in fatjetsmass) and (nleptons > 0) and (leptonpt > 30):
            BOOSTED_COUNT_SIGNAL += 1
            SigGRAPH_BOOSTED.Fill(mychain1.tagjet2eta)
        #HYBRID
        elif (nfatjets > 0) and any(mass > 30 for mass in fatjetsmass) and (nleptons > 0) and (leptonpt > 30):
            HYBRID_COUNT_SIGNAL += 1
            SigGRAPH_HYBRID.Fill(mychain1.tagjet2eta)
...
for jentry in range( entries ):
    ...
    if any(pt > 200 for pt in mychain2.fatjetspt) and any(eta < 2 for eta in mychain2.fatjetseta) and any(Rpt > 20 for Rpt in mychain2.smallRjetspt) and mhiggs > 70 and mhiggs < 140:
        #BOOSTED
        if (nfatjets > 1) and any(mass > 30 for mass in fatjetsmass) and (nleptons > 0) and (leptonpt > 30):
            BOOSTED_COUNT_BACKGROUND += 1
            BackGRAPH_BOOSTED.Fill(mychain2.tagjet2eta)
        #HYBRID
        elif (nfatjets > 0) and any(mass > 30 for mass in fatjetsmass) and (nleptons > 0) and (leptonpt > 30):
            HYBRID_COUNT_BACKGROUND += 1
            BackGRAPH_HYBRID.Fill(mychain2.tagjet2eta)

print(f"BOOSTED BACKGROUND COUNT: {BOOSTED_COUNT_BACKGROUND}\nHYBRID BACKGROUND COUNT: {HYBRID_COUNT_BACKGROUND}")

c1.cd()
SigGRAPH_BOOSTED.GetXaxis().SetTitle("tagjet2eta")
BackGRAPH_BOOSTED.GetXaxis().SetTitle("tagjet2eta")
...
SigGRAPH_HYBRID.GetXaxis().SetTitle("tagjet2eta")
BackGRAPH_HYBRID.GetXaxis().SetTitle("tagjet2eta")

SigGRAPH_HYBRID.Draw('same')
BackGRAPH_HYBRID.Draw('same')
